...
interface ExampleManagerProps {
    examples: Question[];
    onUpdateExamples: (newExamples: Question[]) => void;
}

const ExampleManager: React.FC<ExampleManagerProps> = ({ examples, onUpdateExamples }) => {
    ...
    const handleAdd = (): void => {
        if (newText.trim() && newAnswer !== '') {
            ...
            if (!isNaN(answerNum)) {
                onUpdateExamples([...examples, { text: newText.trim(), answer: answerNum }]);
                setNewText('');
                setNewAnswer('');
                setShow(false);
            }
